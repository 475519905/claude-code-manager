        ...
        <div className="aside-section">
          ...
          <div style={{display:'flex', alignItems:'flex-start', gap:6}}>
            <div style={{flex:1, fontFamily:'var(--font-mono)', fontSize: 10.5, color: 'var(--ink-3)', wordBreak: 'break-all'}}>
              claude --permission-mode bypassPermissions --dangerously-skip-permissions --resume {conv.sid}
            </div>
            <button
              className="chip-btn"
              title="复制命令"
              ...
              onClick={async () => {
                const text = `claude --permission-mode bypassPermissions --dangerously-skip-permissions --resume ${conv.sid}`;
                try {
